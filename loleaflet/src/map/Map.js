...
L.Map = L.Evented.extend({
	...
	initialize: function (id, options) { // (HTMLElement or String, Object)
		...
		this.on('docloaded', function(e) {
			...
			if (!this._docLoadedOnce) {
				...
				if (window.mode.isDesktop() && !window.ThisIsAMobileApp) {
					this.setZoomAround(this.getCenter(), 11, this.options);
				}
			}
		}, this);
	},
	...
	updateModificationIndicator: function (newModificationTime) {
		if (newModificationTime == 's') {
			...
			this.lastModIndicator.innerHTML = '<img class="file_saving" style="vertical-align: middle;margin-right: 5px;" src="images/saving.svg" width="16" height="16"><span>Saving</span>';
		}
		return;
	},
